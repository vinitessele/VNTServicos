...
begin
  dmPrincipal.QuerySelect.Open('SELECT * FROM usuarios WHERE Nome = ' +
    QuotedStr(fPrincipal.lblUsuario.Text) + ' AND Senha =' +
...
begin
  ...
  if (edtUsuarioNome.Text <> '') and
    (edtUsuarioNome.Text <> fPrincipal.lblUsuario.Text) then
  begin
    dmPrincipal.QuerySelect.Open('SELECT * FROM usuarios WHERE Nome = ' +
    ...
    begin
      Executar('UPDATE usuarios SET Nome = ' + QuotedStr(edtUsuarioNome.Text) +
        ',Senha =' + QuotedStr(edtUsuarioSenha.Text) + ' WHERE ID = ' +
        inttostr(edtUsuarioNome.Tag));
      fPrincipal.lblUsuario.Text := edtUsuarioNome.Text;
      Close;
    end;
  end
  else if (edtUsuarioNome.Text = '') then
  begin
